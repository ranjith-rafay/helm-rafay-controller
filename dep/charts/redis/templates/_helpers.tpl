...
*/}}
{{- define "redis.image" -}}
{{ include "common_redis.images.image" (dict "imageRoot" .Values.image "global" .Values.global) }}
{{- end -}}
...
{{ include "common_redis.images.image" (dict "imageRoot" .Values.sentinel.image "global" .Values.global) }}
{{- end -}}
...
{{ include "common_redis.images.image" (dict "imageRoot" .Values.metrics.image "global" .Values.global) }}
{{- end -}}
...
{{ include "common_redis.images.image" (dict "imageRoot" .Values.volumePermissions.image "global" .Values.global) }}
{{- end -}}
...
{{ include "common_redis.images.image" (dict "imageRoot" .Values.sysctl.image "global" .Values.global) }}
{{- end -}}
...
{{- include "common_redis.images.pullSecrets" (dict "images" (list .Values.image .Values.sentinel.image .Values.metrics.image .Values.volumePermissions.image .Values.sysctl.image) "global" .Values.global) -}}
{{- end -}}
...
{{- else -}}
    {{- printf "%s-crt" (include "common_redis.names.fullname" .) -}}
{{- end -}}
{{- end -}}
...
{{- if .Values.serviceAccount.create -}}
    {{ default (include "common_redis.names.fullname" .) .Values.serviceAccount.name }}
{{- else -}}
...
{{- if .Values.master.serviceAccount.create -}}
    {{ default (printf "%s-master" (include "common_redis.names.fullname" .)) .Values.master.serviceAccount.name }}
{{- else -}}
    {{- if .Values.serviceAccount.create -}}
...
{{- if .Values.replica.serviceAccount.create -}}
    {{ default (printf "%s-replica" (include "common_redis.names.fullname" .)) .Values.replica.serviceAccount.name }}
{{- else -}}
    {{- if .Values.serviceAccount.create -}}
        {{ template "redis.serviceAccountName" . }}
    {{- else -}}
...
{{- else -}}
    {{- printf "%s-configuration" (include "common_redis.names.fullname" .) -}}
{{- end -}}
{{- end -}}
...
{{- else -}}
{{- printf "%s" (include "common_redis.names.fullname" .) -}}
{{- end -}}
{{- end -}}
...
{{- include "common_redis.warnings.rollingTag" .Values.image }}
{{- include "common_redis.warnings.rollingTag" .Values.sentinel.image }}
{{- include "common_redis.warnings.rollingTag" .Values.metrics.image }}
{{- end -}}
...
{{ printf "%s.%s" (include "common_redis.names.fullname" .) .Values.useExternalDNS.suffix }}
{{- end -}}
